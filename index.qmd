...
d_display <- d |>
  ...
  mutate(
    highlight_color = case_when(
      session == "exam" ~ "#fcc5f7",   # light yellow
      session == "NC" ~ "#b6fccb",     # light gray
...
d_display_clean |>
  ...
  tab_style(
    style = cell_fill(color = "#fcc5f7"),
    locations = cells_body(rows = d_display$highlight_color == "#fcc5f7")
  ) |>
  # Highlight: no class (gray)
  tab_style(
    style = cell_fill(color = "#b6fccb"),
    locations = cells_body(rows = d_display$highlight_color == "#b6fccb")
  ) |>
